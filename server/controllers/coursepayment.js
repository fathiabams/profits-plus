...
const registerAndPay = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            // If the user doesn't exist, create a new user in the normal User collection
        ...
        const courseUser = await courseuser.findOne({ email, courseid });
        if (courseUser) {
            return res.status(400).json({ message: "User is already registered for this course." });
        }

        // Create a new course user entry
